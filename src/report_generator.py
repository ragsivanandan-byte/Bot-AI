...
def build_merged_guidelines(guidelines_md: str, competitors_md: str,
    ...
    mission = (
        ...
        "4. **Commande terminal prête à coller** — termine TOUJOURS ta réponse "
        "par la ligne EXACTE à copier (avec les vrais noms des variations "
        "gagnantes, pas de placeholder), pour générer les mockups exacts :\n"
        "   ```bash\n"
        "   python automation/make_mockups.py ~/Downloads/<gagnant_design1> "
        "~/Downloads/<gagnant_design2> ~/Downloads/<gagnant_design3>\n"
        "   ```\n"
        "   Remplace chaque `<gagnant_designN>` par le **nom de fichier exact** "
        "de la variation choisie (ex. `NWD_..._01_v3.png`), dans l'ordre "
        "Design 1, Design 2, Design 3 (cf. la liste des variations en section "
        "« Prompts du jour »).\n\n"
